...
func EnsureLogsFolder(folderName string) error {
	...
	if os.IsNotExist(err) {
		...
		err := os.MkdirAll(folderName, 0755) // 0755 adalah mode permission untuk folder baru
		if err != nil {
			return err
		}
...
func Write(description string) error {
	...
	EnsureLogsFolder(fmt.Sprintf("%s/%s", logRoot, folder_year_month))
	...
	fileName := fmt.Sprintf("%s/%s/%s.txt", logRoot, folder_year_month, name)
